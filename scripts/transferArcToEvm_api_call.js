/**
 * Transfer ARC from arc testnet → baseSepolia (Arc to EVM).
 * API at http://localhost:3000. Uses dotenv for PRIVATE_KEY and optional overrides.
 *
 * Usage:
 *   node scripts/transferArcToEvm_api_call.js [amount]
 *
 * .env (or env):
...
async function prepareArcToEvm(userAddress, amount) {
...
async function main() {
  ...
  console.log("Arc testnet → baseSepolia: prepare...");
  const prepared = await prepareArcToEvm(userAddress, amount);
